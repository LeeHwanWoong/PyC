...
public class StaticTypeCheck {
    ...
    public static void V (Expression e, TypeMap tm) {
        ...
        if (e instanceof Binary) {
            ...
            V (b.term2, tm);
            if (b.op.ArithmeticOp( ))
                check( typ1.getId() == typ2.getId() &&
                       (typ1.getId() == "int" || typ1.getId() == "float")
                       , "type error for " + b.op);
        ...
        if (e instanceof Unary) {
            ...
            if (u.op.NotOp()) {
                check((type.getId() == "bool"), "type error for NotOp " + u.op);
            }
            else if (u.op.NegateOp()) {
                check((type.getId() == "int" || type.getId() == "float"), "type error for NegateOp " + u.op);
            }
            return;
        }
